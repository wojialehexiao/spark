...
private[spark] class ExecutorAllocationManager(
  ...
  /**
    * 每隔固定的时间调用一次，以调节未决执行程序请求的数量和正在运行的执行程序的数量。
    *
    * 首先，根据添加时间和当前需求调整我们要求的执行程序。
    * 然后，如果现有执行程序的删除时间已到期，请杀死该执行程序。
    *
    * 这被分解为自己的测试方法。
    */
  private def schedule(): Unit = synchronized {
    ...
    // 重新计算所需的Executor数量，并更新请求Executor的数量
  ...
  /**
    * 更新我们的Executor目标数量，并将结果与​​集群管理器同步。
    *
    * 检查我们现有的分配和以前发出的请求是否超出了我们当前的需求。
    * If so, truncate our target and let the cluster manager know so that it can cancel pending requests that are unneeded.
    *
    * 如果没有，并且添加时间已到期，请查看我们是否可以请求新的执行者并刷新添加时间。
    *
  ...
  private def updateAndSyncNumExecutorsTarget(now: Long): Int = synchronized {
    ...
    //ExecutorAllocationManager 正在初始化，返回0
    if (initializing) {
      // Do not change our target while we are still initializing,
      // Otherwise the first job may have to ramp up unnecessarily
      0
    } else if (maxNeeded < numExecutorsTarget) { //减少需要的Executor数量
