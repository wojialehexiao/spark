...
/**
  ...
  *
  * @note 每个JVM仅应激活一个“ SparkContext”。在创建一个新的SparkContext之前，您必须`stop（）`。
  * @param config 参数是一个描述应用程序配置的Spark Config对象。
  *               此配置中的任何设置都将覆盖默认配置以及系统属性。
  */
class SparkContext(config: SparkConf) extends Logging {
  ...
  //SparkContext的事件总线，可以接收各个使用方法的事件，并且通过异步方式对时间进行匹配后调用SparkListener的不同方法
  private var _listenerBus: LiveListenerBus = _

  //Spark运行时环境， Executor是处理任务的执行器， 它依赖于SparkEnv提供运行时环境。此外，在Driver中也包含了SparkEnv， 这是为了保证local模式下任务的执行
  private var _env: SparkEnv = _

  //提供对作业（job）、阶段（stage）等的监控信息。SparkStatusTracker是一个低级别的API，这意味着只能提供非常脆弱的一致性机制
  private var _statusTracker: SparkStatusTracker = _

  //利用SparkStatusTracker的API， 在控制台显示stage的进度。由于SparkStatusTracker存在一致性问题，所以ConsoleProgressBar在控制台往往有一定延迟
  private var _progressBar: Option[ConsoleProgressBar] = None

  //Spark的用户UI界面， SparkUI间依赖于计算引擎、调度系统、存储体系，作业（job）、阶段（stage）、存储、执行器（Executor）的组件都会以SparkListenerEvent的形式投递到LiveListener中，
  // SparkUI将从哪个各个SparkListener中读取数据并显示到Web
  ...
  //TaskScheduler 任务调度器， 是调度系统中重要组件之一。 TaskScheduler按照调度算法对集群已经分配给应用的资源进行二次调度分配给任务。
  //TaskScheduler调度的任务是由DAGScheduler创建的，所以DAGScheduler是TaskScheduler的前置任务
  private var _taskScheduler: TaskScheduler = _

  //心跳接收器。 所有Executor都会向HeartbeatReceiver发送心跳信息， _heartbeatReceiver接收到心跳信息后，
  //首先更新Executor的最后可见时间， 然后将此信息交给TaskScheduler作进一步处理
  private var _heartbeatReceiver: RpcEndpointRef = _

  // DAG调度器，是调度系统中重要组件之一, 负责创建Job，将DAG中的RDD划分到不同的Stage、提交Stage等。
  ...
  try {
    ...
    // 需要将Driver的指标系统设置为spark.app.id到应用程序ID。
    ...
    //创建和启动 ExecutorAllocationManager， 其为根据工作负载，动态添加和删除Executor
    //判断是否开启动态负载
    ...
    /**
      * 向事件总线投递 SparkListenerApplicationStart事件
      */
  ...
  private def postEnvironmentUpdate(): Unit = {
    if (taskScheduler != null) {
      ...
      /**
        * 生成SparkListenerEnvironmentUpdate时间， 并投送到事件总线
        */
...
object SparkContext extends Logging {
  ...
  /**
    ...
    *
    *
    * 设置SparkContext为激活，并保存到activeContext
    */
