...
abstract class RDD[T: ClassTag](
  ...
  /**
    ...
    * 用于获取当前RDD的所有依赖的序列
    * 从checkpoint中获取RDD，并将这些RDD封装为OneToOneDependency列表。
    * 如果获取到，则返回RDD的依赖
    * 否则调用子类实现的getDependencies方法获取当前RDD的依赖后赋予dependencies， 最后返回dependencies_
    */
  ...
  final def partitions: Array[Partition] = {
    ...
    //partition是查找分区数组的优先级为： checkoutpoint -> 读取partitions_ -> 调用getPartition是方法
  ...
  /**
    ...
    * RDD is checkpointed.
    * 优先调用CheckPoint中保存的RDD的getPreferredLocations获取分区
    * 如果没有，调用自身getPreferredLocations获取分区偏好位置
    */
  ...
  /**
    * 是否已经调用doCheckpoint方法设置检查点。此属性可以阻止RDD多次设置检查点
    */
