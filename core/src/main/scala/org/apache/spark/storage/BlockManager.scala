...
/**
  ...
  * 用于封装从本地的BlockManager中获取的Block数据及Block相关联的度量数据
  */
private[spark] class BlockResult(
                                  //Block数据及Block相关联的度量数据
                                  val data: Iterator[Any],
                                  //读取Block的方法。提供Memory, Disk, Hadoop, Network四个枚举值
...
private[spark] class BlockManager(
                                   ...
                                   externalBlockStoreClient: Option[ExternalBlockStoreClient]) extends BlockDataManager with BlockEvictionHandler with Logging {
      ...
      doPut(blockId, level, classTag, tellMaster, keepReadLock) { info =>
        ...
        val replicationFuture = if (level.replication > 1) {
          Future {
            ...
            // thread pool.
            //创建异步线程通过调用replicate方法复制Block数据到其他节点的储存体系中
  ...
  def initialize(appId: String): Unit = {
    ...
    //BlockManager在本地生成的BlockManagerId实际上只是在向BlockManagerMaster注册BlockManager时， 给BlockManagerMaster提供参考
    //BlockManagerMaster会创建一个包含了拓扑信息的新BlockManagerId正式分配给BlockManager的身份标识
    val id =
    ...
    //生成shuffleServerId
    //当启用了外部Shuffle服务， 并且当前BlockManager所在节点不是Driver时，需要注册外部Shuffle服务
  ...
  def reregister(): Unit = {
    ...
    //向BlockManagerMaster报告所有的Block信息
    reportAllBlocks()
  }
  ...
  /**
    ...
    *
    * 从储存体系获取BlockId所对应Block数据。封装为ManagedBuffer
    */
  override def getBlockData(blockId: BlockId): ManagedBuffer = {

    //如果是ShuffleBlock，调用ShuffleManager的shuffleBlockResolver组件的getBlockData
    if (blockId.isShuffle) {
      shuffleManager.shuffleBlockResolver.getBlockData(blockId.asInstanceOf[ShuffleBlockId])
    } else {
      //如果不是ShuffleBlock，调用getLocalBytes获取本地数据，如果获取不到， 调用reportBlockStatus告诉BlockManagerMaster此block不存在
  ...
  private def reportBlockStatus(
    ...
    //调用tryToReportBlockStatus方法向BlockManagerMaster汇报BlockStatus
    val needReregister = !tryToReportBlockStatus(blockId, status, droppedMemorySize)

    //如果返回true，则说明需要重新注册该BlockManager
    if (needReregister) {
  ...
  private def doGetLocalBytes(blockId: BlockId, info: BlockInfo): BlockData = {
    ...
    //未序列化， 按照diskStore、memoryStore获取
    if (level.deserialized) {
  ...
  /**
    ...
    * 获取远端BlockManager以序列化的字节形式获取Block数据
    *
    */
  private[spark] def getRemoteValues[T: ClassTag](blockId: BlockId): Option[BlockResult] = {
    ...
    getRemoteBlock(blockId, (data: ManagedBuffer) => {
      ...
      val values = serializerManager.dataDeserializeStream(blockId, data.createInputStream())(ct)
  ...
  private[spark] def getRemoteBlock[T](
    ...
    //从master获取位置和状态信息 BlockLocationsAndStatus
    ...
    if (locationsAndStatusOption.isEmpty) {
      ...
      //从在同一主机上运行的另一个执行程序的本地目录中读取该块
      locationsAndStatus.localDirs.flatMap { localDirs =>
        val blockDataOption = readDiskBlockFromSameHostExecutor(blockId, localDirs, locationsAndStatus.status.diskSize)
  ...
  /**
    ...
    *
    * 返回给定块的位置列表，优先处理本地计算机，因为多个块管理器可以共享同一主机，然后可以共享同一机架上的主机。
    * 在以上列出的每个组（主机，机架和其他主机）中，执行者优先于外部洗牌服务。
    */
    ...
    while (locationIterator.hasNext) {
      ...
      val data = try {
        ...
        case NonFatal(e) =>
          ...
          // large number of stale entries causing a large number of retries that may take a significant amount of time.
          // To get rid of these stale entries we refresh the block locations after a certain number of fetch failures

          //如果执行者数量众多，则位置列表可能包含大量陈旧条目，从而导致大量重试，这可能会花费大量时间。
          //为了摆脱这些过时的条目，我们会在一定数量的提取失败后刷新块位置
  ...
  /**
    ...
    *
    * 获取Block。如果存在，则获取此Block并返回BlockResult，否则调用makeIterator方法计算Block，
  ...
  /**
    ...
    * cases.
    *
    * 获取DiskBlockObjectWriter， 通过DiskBlockObjectWriter可以跳过对DiskStore的使用
    * 直接写入磁盘
    */
  def getDiskWriter(
                     blockId: BlockId,
  ...
  def putBytes[T: ClassTag](
    ...
    val blockStoreUpdater = ByteBufferBlockStoreUpdater(blockId, level, implicitly[ClassTag[T]], bytes, tellMaster)
  ...
  private def removeBlockInternal(blockId: BlockId, tellMaster: Boolean): Unit = {
    ...
    //如果需要向BlockManagerMaster汇报。则调用reportBlockStatus
    if (tellMaster) {
