 ...
 *
 * 获取多个Block的迭代器， 如果在本地，从本地BlockManager获取
 * 如果在远程，从远程获取BlockTransferService
 *
...
final class ShuffleBlockFetcherIterator(
                                         ...
                                         //单次航班（即一批）可以获取的最大请求的最大字节数
                                         //单个请求不能超过该值的1/5，这是为了保证至少有5个并发
  ...
  private[this] def splitLocalRemoteBlocks(): ArrayBuffer[FetchRequest] = {
    ...
    for ((address, blockInfos) <- blocksByAddress) {
      ...
      //如果blockManagerId对应的executorId与当前executorId相同，则说明为本地
      if (address.executorId == blockManager.blockManagerId.executorId) {
        ...
        // 对剩余的Block新建FetchRequest放入remoteRequests
  ...
  private[this] def initialize(): Unit = {
    ...
    // 划分从本地还是从远程获取的Block氢气
    val remoteRequests = splitLocalRemoteBlocks()


    // Add the remote requests into our queue in a random order
    // 将remoteRequest随机排序后放入fetchRequests
